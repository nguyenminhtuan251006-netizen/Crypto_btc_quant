...
# The 12 HFT feature column names (aligned with Oxford paper microstructure)
HFT_FEATURE_COLS = [
...
def load_hft_features(hft_path: str = None) -> pd.DataFrame:
    ...
    # Depth-weighted OBI (Albers et al. 2021 Oxford paper: decay with depth)
    if "obi_weighted" not in df_hft.columns and "obi_l1" in df_hft.columns:
        df_hft["obi_weighted"] = 0.5 * df_hft["obi_l1"] + 0.3 * df_hft["obi_l5"] + 0.2 * df_hft["obi_l20"]
