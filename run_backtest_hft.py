...
def main():
    ...
    signals_v1 = np.where(is_active_v1 & (test_v1["trend_dist"].to_numpy() > 0), signals_v1_raw, 0)
    ...
    signals_v2 = np.where(is_active_v2 & (test_v2["trend_dist"].to_numpy() > 0), signals_v2_raw, 0)
    ...
    engine = BinanceFuturesEngine(
        ...
        leverage=5.0,
        fee_rate=0.00015,     # 0.015% Maker fee
        stop_loss_pct=0.005,  # 0.5% SL
        take_profit_pct=0.010 # 1.0% TP (2:1 R/R)
    )
    ...
    # V1 Backtest
    df_res_v1, trades_v1 = engine.run(test_v1, signals_v1)
    m_v1 = compute_crypto_metrics(df_res_v1, trades_v1, engine.initial_capital)
    ...
    # V2 Backtest
    df_res_v2, trades_v2 = engine.run(test_v2, signals_v2)
    m_v2 = compute_crypto_metrics(df_res_v2, trades_v2, engine.initial_capital)
    m_v2["Model"] = "V2: HFT Microstructure"
    m_v2["Features"] = f"{len(v2_cols)} (candles + HFT)"
    m_v2["Accuracy (%)"] = round(acc_v2, 2)
    results.append(m_v2)
    ...
    if len(trades_v2) > 0:
        trades_v2.to_csv(os.path.join(reports_dir, "v2_hft_trades_log.csv"), index=False)

    print(f"\n[Saved] Reports → {reports_dir}/")
    print(f"  - hft_upgrade_comparison.csv")
    if len(trades_v2) > 0:
        print(f"  - v2_hft_trades_log.csv ({len(trades_v2)} trades)")
